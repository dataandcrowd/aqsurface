...
The development version (v0.0.1, lifecycle stage: experimental)
is hosted at <https://github.com/dataandcrowd/aqsurface>.
Submission to CRAN is planned subject to peer review of the
present manuscript.

All cached intermediate results required by the tables and
figures of this article are committed to the repository
(`paper/*.rds`, `paper/figures/*.png`, and
`inst/extdata/benchmark_*.rds`). Rendering the manuscript from a
fresh clone therefore requires no heavy computation and takes
under one minute on a modern laptop:
...
The cached results can be regenerated by running
`source("paper/figures.R")` (2-3 minutes). The full
PM10 and NO2 sweeps that back @tbl-robustness and
@fig-boxplot can be re-run via
`source("data-raw/run_benchmark_pm10.R")` and
`source("data-raw/run_benchmark_no2.R")` (approximately five
minutes each); both scripts require the legacy archives
`pm10.RData`, `no2.RData`, and `stations_10km.shp`, whose
location is read from the `AQSURFACE_DATA_DIR` environment
variable (see `?aqs_data_dir`). The Jan S1 demo bundle that
ships with the package is sufficient for every other table and
